const Kai = (function () {
      ...
      for (var i in options) {
        if (allow.indexOf(i) !== -1) {
          if (i === "methods") {
            ...
            this[i] = JSON.parse(
              JSON.stringify(typeof options[i] === "object" ? options[i] : {})
            );
          } else {
            this[i] = options[i];
          }
  ...
  Kai.prototype.addKeydownListener = function () {
    ...
    if (this._router) {
      ...
      document.addEventListener(
        "keydown",
        this.handleLocalKeydown.bind(this),
        true
      );
    }
  };
  ...
  Kai.prototype.nav = function (next, navIndex, navClass) {
    ...
    if (navClass === "horizontalNavClass") {
      return (targetElement.parentElement.scrollLeft =
      ...
      if (targetElement.offsetTop > targetElement.parentElement.clientHeight) {
        var fill = 0;
        var scroll =
          targetElement.offsetTop - targetElement.parentElement.clientHeight;
